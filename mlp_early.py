...
# -----------------------------------------------------------
# 4. Early Concat + MLP 回归模型
# -----------------------------------------------------------
def build_early_concat_mlp(input_dim, embed_dim=256):
    ...
    return models.Model(inp, out, name="EarlyConcatMLP")

# 注意输入特征长度
input_dim = IMG_SIZE * IMG_SIZE * 3 + MEL_SIZE * MEL_SIZE
model = build_early_concat_mlp(input_dim)
...
class InspectPred(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        feat, lbl = next(val_ds.unbatch().take(1).as_numpy_iterator())
        pred = self.model.predict(feat[None], verbose=0)[0, 0]
...
ckpt_path = "best_early_concat_mlp_mae.keras"
...
y_true, y_pred = [], []
for feat, lbl in val_ds:
    y_true.extend(lbl.numpy().ravel())
    y_pred.extend(best_model.predict(feat, verbose=0).ravel())
...
best_model.save("xigua_early_concat_mlp_best.keras")
...
ckpt_path = "xigua_early_concat_mlp_best.keras"
